...
typedef struct {
  ...
  double beamwidth; /*width of gaussian beam*/
  int bd;           /*upper or lower boundary*/
  double ca1;       /*cosine of input angle*/
...
typedef struct {
  ...
  double r;                /* total radius */
  short source_type;              /*input source type*/

  double h;        /*height of source from the 1st layer of artery*/
  double divergence; /*Beam dievrgence for a Gaussian beam*/
  double beamwaist;  /*Beam waist for a Gaussian beam*/
  double    Rsp_onerun; /*specular reflectance in each run */
...
typedef struct {
  ...
  double ** Rd_xy;	/* 2D distribution of diffuse */
					/* reflectance. [1/(cm2 sr)] */
  double *  Rd_y;	/* 1D radial distribution of diffuse */
					/* reflectance. [1/cm2] */
  double *  Rd_x;	/* 1D angular distribution of diffuse */
					/* reflectance. [1/sr] */
  double    Rd;		/* total diffuse reflectance. [-] */

  double *** A_xyz;	/* 3D probability density in turbid */
					/* media over x,z & z. [1/cm3] */
  double ** A_xz;	/* 2D probability density over x and z. */
  ...
  double ** Tt_xy;	/* 2D distribution of total */
					/* transmittance. [1/(cm2 sr)] */
  double *  Tt_y;	/* 1D radial distribution of */
					/* transmittance. [1/cm2] */
  double *  Tt_x;	/* 1D angular distribution of */
					/* transmittance. [1/sr] */
...
double ***Alloc3D(short, short,short, short ,short , short);
void 	 FreeVector(double *, short, short);
void 	 FreeMatrix(double **, short, short, short, short);
void     Free3D(double ***,short ,short ,short ,short , short , short );
void 	 nrerror(char *);
