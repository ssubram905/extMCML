...
void LaunchPhoton(InputStruct *, LayerStruct *, PhotonStruct *);
...
{  register long i_photon;
  ...
  do {
    ...
    LaunchPhoton(In_Ptr, In_Ptr->layerspecs,  &photon);
    out_parm.Rsp += In_Ptr->Rsp_onerun;
    while (!photon.dead) HopDropSpin(In_Ptr, &photon, &out_parm);
} while(--i_photon);
...
main(int argc, char *argv[])
{
  ...
  ShowVersion("Version 2.0.1, 2013");
